...
struct _event_queue_imp
{
			...
			const media_timed_event* FindFirstMatch(bigtime_t eventTime,
										BTimedEventQueue::time_direction
										direction, bool inclusive,
										int32 eventType);

			status_t			DoForEach(BTimedEventQueue::for_each_hook hook,
									void* context, bigtime_t eventTime,
									BTimedEventQueue::time_direction direction,
									bool inclusive, int32 eventType);

			void				SetCleanupHook(BTimedEventQueue::cleanup_hook
									hook, void* context);
...
private:
	struct event_queue_entry {
		struct event_queue_entry* previous;
		struct event_queue_entry* next;
		media_timed_event event;
	};

private:
			void				_RemoveEntry(event_queue_entry *entry);
			void 				_CleanupEvent(media_timed_event *event);

			event_queue_entry*	_GetEndBeforeTime(bigtime_t eventTime,
									bool inclusive);
			event_queue_entry*	_GetStartAfterTime(bigtime_t eventTime,
									bool inclusive);
